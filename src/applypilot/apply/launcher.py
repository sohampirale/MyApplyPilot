...
the database, launches Chrome + browser-use agent for each one, parses the
...
# Track active browser-use agents for skip (Ctrl+C) handling
from applypilot.apply.agent import run_agent_sync, cancel_agent, cancel_all_agents
...
# Max steps for the browser-use agent per job application
_AGENT_MAX_STEPS = 100
        ...
        if not job_data.get("tailored_resume_path") and job_data.get("full_description"):
            try:
                ...
                if tailored and status in ("approved", "approved_with_judge_warning"):
                    safe_title = re.sub(r"[^\w\s-]", "", job_data.get("title") or "job")[:50].strip().replace(" ", "_")
                    safe_site = re.sub(r"[^\w\s-]", "", job_data.get("site") or "site")[:20].strip().replace(" ", "_")
                    prefix = f"{safe_site}_{safe_title}"
                    txt_path = TAILORED_DIR / f"{prefix}.txt"
                    txt_path.write_text(tailored, encoding="utf-8")
                    try:
                        job_data["tailored_resume_path"] = str(convert_to_pdf(txt_path))
                    except Exception:
                        job_data["tailored_resume_path"] = str(txt_path)
...
def run_job(job: dict, port: int, worker_id: int = 0,
    ...
    """Run a browser-use agent for one job application.
    ...
    try:
        # Action callback for real-time dashboard updates
        def _on_action(action_desc: str) -> None:
        ...
        # Save full transcript to log file
        config.ensure_dirs()
        ...
        try:
            with open(worker_log, "a", encoding="utf-8") as lf:
                lf.write(log_header)
                lf.write(output)
                lf.write(f"\n--- {action_count} actions in {elapsed}s ---\n")
...
def main(limit: int = 1, target_url: str | None = None,
    ...
    def _sigint_handler(sig, frame):
        ...
        if _ctrl_c_count == 1:
            ...
            # Cancel all active browser-use agents to skip current jobs
            cancel_all_agents()
        else:
